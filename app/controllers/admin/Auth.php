...
class Auth extends MY_Controller
{
    ...
    /**
     * Create a superadmin user with admin privileges.
     * Accessible only to Owner (main admin).
     * Usage: /admin/auth/create_superadmin
     */
    public function create_superadmin()
    {
        if (!$this->Owner) {
            show_error('Access denied. Only Owner can create superadmin.');
        }
        $this->load->admin_model('settings_model');
        $group = $this->settings_model->getGroupByName('admin');
        if (!$group) {
            show_error('Admin group not found.');
        }
        $group_id = $group->id;
        ...
        $user_id = $this->ion_auth->register($username, $password, $email, $additional_data, $active);
        if ($user_id) {
