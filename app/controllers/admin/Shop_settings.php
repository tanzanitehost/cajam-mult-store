...
class Shop_settings extends MY_Controller
{
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($_FILES['logo']['size'] > 0) {
                ...
                if (!$this->upload->do_upload('logo')) {
                    ...
                    $img_config['source_image']   = $this->upload_path . 'logos/' . $logo;
    ...
    public function sms_settings()
    {
        ...
        if ($this->form_validation->run() == true && $this->shop_admin_model->updateSmsSettings($sms_config)) {
            ...
            // Ensure we always have a valid object with a config object
            if (!$sms_settings) {
                $sms_settings = (object) [
                    'auto_send' => 0,
                    // store as JSON string initially; will decode below
                    'config'    => json_encode(['gateway' => 'Log']),
                ];
            }

            // Decode config and provide safe defaults
            $config = null;
            if (isset($sms_settings->config)) {
                $config = is_string($sms_settings->config)
                    ? json_decode($sms_settings->config)
                    : (is_object($sms_settings->config) ? $sms_settings->config : null);
            }
            if (!$config || !is_object($config)) {
                $config = (object) ['gateway' => 'Log'];
            }
            if (empty($config->gateway)) {
                $config->gateway = 'Log';
            }
            $sms_settings->config       = $config;
